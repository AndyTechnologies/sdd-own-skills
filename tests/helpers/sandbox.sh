...
init_sandbox() {
  ...
  # idempotencia con caches parciales dentro del HOME. Se reusa el cache del
  # HOST (que el warm build del suite mantiene caliente) y se prohibe fetch,
  # proxy, telemetria y env files de go.
  if command -v go >/dev/null 2>&1; then
    SB_ENV+=(
      GOMODCACHE="$(go env GOMODCACHE 2>/dev/null || true)"
      GOCACHE="$(go env GOCACHE 2>/dev/null || true)"
      GOPROXY=off GOSUMDB=off GOTELEMETRY=off GOENV=off
    )
  fi
}
...
snapshot_tree() {
  ( cd "$1" && find . -type f \
      ...
      -not -path './go/*' \
      -exec md5sum {} \; -exec stat -c '%n %Y' {} \; | sort ) > "$2"
}
