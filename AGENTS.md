...
- **Paso 1 — install de lo nuevo en `~/.config/sdd-own/`** (el canónico de TODO lo nuestro):
  ...
  - `wiring/prompts/sdd/{orchestrator.md,sdd-rfc-author.md,sdd-council.md}` → **originales** en `~/.config/sdd-own/prompts/sdd/` + symlinks por archivo en `~/.config/opencode/prompts/sdd/` y `~/.claude/prompts/sdd/` apuntando al original. Alan no gestiona esos 2 paths; los prompts de fase de Alan en ese mismo dir NO se tocan.
...
- **Excepción sancionada (R1)**: el fragmento porta UNA única llave de runtime además de las claves SDD: `subagent_depth` (top-level, entero). Es la llave mínima que habilita el encadenamiento profundo de sub-agentes SDD (council → lens). El fragmento NUNCA porta `providers`, `mcp`, `permission`, `share`, `model` ni llaves de otros agentes. `subagent_depth` se mergea fragment-wins e idempotente en AMBOS motores del sync (jq y python fallback) — ver `sync-skills.sh`; el valor previo del usuario queda respaldado en el `.bak`.
