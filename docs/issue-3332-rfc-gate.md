...
- `commands/sdd-continue.md` — 'QUEST CONDITIONAL' gatillea en `## Approval:` (approved → SKIP quest y pasar a explore; needs-changes → re-run; rejected → NO continuar, reportar).    Flujo: quest → explore → propose → [spec ∥ design] → tasks → apply → verify → archive.
- `opencode.json` — define el subagente `sdd-rfc-author` (rol autor del RFC) y lo añade a la allowlist de tasks del orquestador (para que pueda lanzarlo). El routing por fase (QUEST CONDITIONAL) vive en `commands/sdd-continue.md`, no en una tabla dentro de opencode.json.
